...
The **cheap-contract track is backtested the same way**: among players flagged sub-replacement, "Overpay (dead money)" contracts should cost far more per realized unit than "Fair (min contract)" ones while delivering comparable (low) production. On held-out seasons they do — dead-money deals run ≈ 5–6× the realized $/value of fair-minimum deals for essentially the same output — so the verdicts that lifted coverage from 32% to 80% are earned, not asserted.
